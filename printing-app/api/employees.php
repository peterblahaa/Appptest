...
switch ($method) {
    ...
    case 'POST':
        ...
        $stmt = $conn->prepare("INSERT INTO employees (id, name, email, role, joinedDate, department, active, color) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt->execute([$id, $name, $email, $role, $joinedDate, $department, $active, $color])) {
